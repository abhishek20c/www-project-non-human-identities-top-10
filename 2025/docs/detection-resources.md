...
| [gh-iga](https://github.com/abhishek20c/gh-iga) | NHI1, NHI5 | GitHub org members, outside collaborators, and service/bot accounts held as org identities | GitHub | `repo` + `read:org`; read-only (yes) | Stale privileged accounts (inactive 90+ days still holding write/admin), admin sprawl, privileged outside collaborators, over-permissioned repos, orphaned members, redundant direct grants | Activity is a proxy (commit/PR/review only), so dormant service accounts may show as inactive; does not yet inventory GitHub Apps, OAuth apps, or PATs (roadmap); no secret scanning | All processing is local; the token is never written to disk or included in report output; no telemetry or outbound calls | MIT; actively maintained |
